    ...
    if (mstActive && mstCurrentStep < (int)mstSteps.size()) {
        ...
        if (mstStepTimer <= 0.0f) {
            ...
            if (mstCurrentStep < (int)mstSteps.size()) {
                ...
                char buf[64];
                std::snprintf(buf, sizeof(buf), "Hoan thanh! Tong trong so MST: %d", total);
...
void GraphScreen::Draw() const {
    ...
    if (mstActive) {
        char stepBuf[48];
        std::snprintf(stepBuf, sizeof(stepBuf), "Buoc %d / %d", mstCurrentStep, (int)mstSteps.size());
...
void GraphScreen::RunKruskal() {
    ...
    mstActive = true;
    mstStepTimer = 0.8f;
    SetMsg("Kruskal: dang chay tung buoc...", Pal::TxtMid, 3.0f);
}
...
void GraphScreen::RunPrim() {
    ...
    mstActive = true;
    mstStepTimer = 0.8f;
    SetMsg("Prim: dang chay tung buoc...", Pal::TxtMid, 3.0f);
}
